...
class Boss:
    def __init__(self, name, title, character, hitpoints, dmg, armor, bossLevel, specialTimer):
        self.name = name
        self.title = title
    ...
    def GetMapLine(self):
        ...
        if lines[0] == "k":
            ...
            line = lines[0]
            self.CalculateWaitTime(float(line))
    ...
    def UpdateHitTime(self):
        for array in self.hitList:
            array[3] = array[3] - 1
            if array[3] <= 0: self.hitList.remove(array)
